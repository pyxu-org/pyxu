...
To overcome these limitations, more sophisticated imaging algorithms were independently developed across various communities. These innovations significantly advanced their respective fields by utilizing advanced iterative reconstruction methods or specialized computation strategies to evaluate $\mathcal{F}$ and related operators. For instance, in both Magnetic Resonance Imaging (MRI) and Radio Astronomy, iterative algorithms that exploit efficient Fourier sampling techniques on non-uniform grids have been introduced [@Greengard:2004, @Arras:2021, @Wang:2023]. Similarly, developments in efficient implementations of convolution operators have been pivotal for deblurring images in microscopy and astronomy. Moreover, advances in tomographic operators have emerged in parallel for several critical applications, including medical imaging, materials science, and plasma or geophysical imaging [@O'Connell:2021, @Koneti:2019, @Kim:2006, @Rawlinson:2005]. Despite their apparent differences, the majority of imaging problems can be categorized into a few forms such as Fourier sampling, spatial sampling, and tomographic sampling. Consequently, once domain-specific terminology is set aside, the reconstruction methods across fields reveal strong interconnections.
