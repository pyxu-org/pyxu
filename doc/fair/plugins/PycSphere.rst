.. _plugins-page:

.. |br| raw:: html

   </br>

.. raw:: html

    <!-- CSS overrides on the pyxu-fair only -->
    <style>

    .summaryinfo {
        color: #000;
        font-size: 80%;
        margin-bottom: 12px;
        margin-top: 12px;
    }

    .entrypointraw {
        color: #777;
    }
    .badge {
        white-space: nowrap;
        display: inline-block;
        vertical-align: middle;
        /*vertical-align: baseline;*/
        font-family: "DejaVu Sans", Verdana, Geneva, sans-serif;
        /*font-size: 90%;*/
    }
    .currentstate {
        color: #666;
        font-size: 90%;
        margin-bottom: 12px;
    }
    span.badge-left {
        border-radius: .25rem;
        border-top-right-radius: 0;
        border-bottom-right-radius: 0;
        color: #212529;
        background-color: #A2CBFF;
        /* color: #ffffff; */
        text-shadow: 1px 1px 1px rgba(0,0,0,0.3);

        padding: .25em .4em;
        line-height: 1;
        text-align: center;
        white-space: nowrap;
        float: left;
        display: block;
    }

    span.badge-right {
        border-radius: .25rem;
        border-top-left-radius: 0;
        border-bottom-left-radius: 0;

        color: #fff;
        background-color: #343a40;

        padding: .25em .4em;
        line-height: 1;
        text-align: center;
        white-space: nowrap;
        float: left;
        display: block;
    }

    .badge-right.light-blue, .badge-left.light-blue {
        background-color: #A2CBFF;
        color: #212529;
    }

    .badge-right.light-red, .badge-left.light-red {
        background-color: rgb(255, 162, 162);
        color: rgb(43, 14, 14);
    }

    .badge-right.red, .badge-left.red {
        background-color: #e41a1c;
        color: #fff;
    }

    .badge-right.blue, .badge-left.blue {
        background-color: #377eb8;
        color: #fff;
    }

    .badge-right.green, .badge-left.green {
        background-color: #4daf4a;
        color: #fff;
    }

    .badge-right.purple, .badge-left.purple {
        background-color: #984ea3;
        color: #fff;
    }

    .badge-right.orange, .badge-left.orange {
        background-color: #ff7f00;
        color: #fff;
    }

    .badge-right.brown, .badge-left.brown {
        background-color: #a65628;
        color: #fff;
    }

    .badge-right.dark-gray, .badge-left.dark-gray {
        color: #fff;
        background-color: #343a40;
    }


    .badge a {
        text-decoration: none;
        padding: 0;
        border: 0;
        color: inherit;
    }

    .badge a:visited, .badge a:active {
        color: inherit;
      }

    .badge a:focus, .badge a:hover {
        color: rgba(255,255,255,0.5);
        mix-blend-mode: difference;
        text-decoration: none;
        /* background-color: rgb(192, 219, 255); */
    }


    .svg-badge {
        vertical-align: middle;
    }

    .tooltiptext {
        visibility: hidden;
        /* width: 120px; */
        background-color: rgb(255, 247, 175);
        color: #000;
        text-align: center;
        border-radius: 6px;
        padding: 5px;

        /* Position the tooltip */
        position: absolute;
        z-index: 1;
    }

    .tooltip:hover .tooltiptext {
        visibility: visible;
    }
    </style>

*********
PycSphere
*********


Description
===========

.. raw:: html
    Adipisci etincidunt sit velit ipsum adipisci. Neque aliquam ipsum voluptatem sit. Numquam porro sit velit labore labore quisquam quisquam. Ipsum quaerat eius sed eius aliquam labore dolorem. Quisquam velit tempora quaerat adipisci. Ut porro etincidunt est. Dolorem porro non numquam. Est dolor modi neque sed.

    Voluptatem dolor magnam dolorem tempora etincidunt. Neque dolor neque ut amet non voluptatem voluptatem. Velit dolorem magnam eius. Dolore porro quisquam voluptatem quisquam dolorem ut dolorem. Quaerat velit aliquam ipsum porro etincidunt. Sit tempora ut sed ut aliquam magnam. Quisquam tempora est consectetur ipsum. Sit non sed amet neque eius. Consectetur aliquam est ut labore. Quisquam tempora non sed magnam sed dolorem.

    Eius est non dolore voluptatem tempora. Amet numquam sit adipisci est eius magnam. Magnam voluptatem neque aliquam neque dolore voluptatem voluptatem. Neque dolor eius est dolorem neque. Quaerat dolore quisquam quisquam. Sed dolore neque consectetur quisquam. Porro quisquam est numquam etincidunt non. Modi amet numquam amet est.

    Modi modi eius etincidunt est etincidunt sed. Est eius neque eius ipsum quaerat ipsum labore. Ut sed eius labore neque porro. Consectetur sit sit dolore quisquam sed adipisci porro. Porro labore modi quiquia numquam etincidunt dolore voluptatem. Dolor quisquam adipisci dolore neque. Velit quaerat eius ipsum velit.


General information
===================

.. raw:: html

    
    <p>
        <strong>Short description</strong>: Inverse problems on the sphere
    </p>
    

    <p class="currentstate">
        
        <img class="svg-badge" src="../../_static/plugins/status/status-planning-d9644d.svg" title="Pre-alpha: Not yet ready to use. Developers welcome!"></p>
    

    <p>
        <strong>Source code</strong>: <a href="https://github.com/jhawk/PycSphere" target="_blank">Go to the source code repository</a>
    </p>

    
    <p>
        <strong>Documentation</strong>: <a href="https://jhawk.github.io/PycSphere/html/index" target="_blank">Go to plugin documentation</a>
    <p>
    


Detailed information
====================

.. raw:: html
    
    <p>
        <strong>Author(s)</strong>: Jeri Hawk
    </p>
    
    
    <p>
        <strong>Contact</strong>: <a href="mailto:jeri.hawk@pyxu_user.org">jeri.hawk@pyxu_user.org</a>
    </p>
    
    
    <p>
        <strong>Most recent version</strong>: 0.1.0
    </p>
    <p>
        <strong>Compatibility</strong>:
        <img class="svg-badge" title="Compatible with Pyxu 2.0.0" src="https://img.shields.io/badge/Pyxu-2.0.0-007ec6.svg?logo=data:image/png;base64,">
    </p>
    


Components contributed
======================

.. raw:: html
    <p class="summaryinfo">
        
        <span class="badge">
            <span class="badge-left blue">Operator</span>
            <span class="badge-right">1</span>
        </span>
        
        <span class="badge">
            <span class="badge-left brown">Solver</span>
            <span class="badge-right">1</span>
        </span>
        
        <span class="badge">
            <span class="badge-left purple">Stop</span>
            <span class="badge-right">2</span>
        </span>
        
    </p>
    
    
    
      Operator <span class="entrypointraw">(pyxu.operator)</span>
    
    <ul>
    
    <li><code>fancy_operator</code>
    
    </ul>
    
    
      Solver <span class="entrypointraw">(pyxu.solver)</span>
    
    <ul>
    
    <li><code>half_solver</code>
    
    </ul>
    
    
      Stop <span class="entrypointraw">(pyxu.stop)</span>
    
    <ul>
    
    <li><code>spooky_stop</code>
    
    <li><code>ritzy_stop</code>
    
    </ul>
    
    