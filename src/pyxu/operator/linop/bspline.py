...
compare it with a purely discrete model where the non-uniformity of the sampling pattern is not accounted for, which
leads to much worse reconstruction results. In both cases, we use total-variation (TV) regularization :math:`||\nabla
\cdot||_{2, 1}`, where :math:`\nabla` is the gradient operator and :math:`||\cdot ||_{2, 1}` is the mixed
:math:`L_{2,1}` norm (isotropic TV). In the B-spline model, the continuous gradient is evaluated exactly on the pixel
grid, whereas it can only be approximated using finite differences in the discrete model.
...
.. code-block:: python3
   ...
   # Data fidelity term
   f = 1 / 2 * pxo.SquaredL2Norm(dim=sampling_op.codim).asloss(y) * sampling_op
   ...
   K.lipschitz()
   h = pxo.L21Norm(arg_shape=(2, N, N))
   lamb = 2*1e-2  # Regularization parameter

   # Stopping criterion
   tol = 1e-4
   stop_crit = pxst.RelError(eps=tol, var="x") & pxst.RelError(eps=tol, var="z")

   # Solve optimization problem
   solver = pxs.CV(f=f, g=g, h=lamb * h, K=K, verbosity=1000)
   ...
   f_d = 1 / 2 * pxo.SquaredL2Norm(dim=sampling_op.codim).asloss(y)  # Forward model is identity
   K_d = pxo.Gradient(image.shape)
   K_d.lipschitz()
   h_d = pxo.L21Norm(arg_shape=(2, N, N))

   # Solve optimization problem
   solver_d = pxs.CV(f=f_d, h=lamb * h_d, K=K_d, verbosity=1000)
...
Below is the same example with Hessian-Schatten regularization :math:`||\mathbf{H} \cdot||_{1, *} = \sum_{i=1}^N
||\mathbf{H}_i \cdot ||_{*}`, where :math:`\mathbf{H} = (\mathbf{H}_0, \ldots, \mathbf{H}_{N-1})` is the Hessian
operator where :math:`\mathbf{H}_i` is the Hessian at the :math:`i`-th pixel and :math:`||\cdot ||_*` is the 1-Schatten
norm for matrices (or nuclear norm). This type of regularization was first introduced in [HessianSchatten]_.  In the
B-spline model, the continuous Hessian is evaluated exactly on the pixel grid, whereas it can only be approximated using
finite differences in the discrete model.
...
* This example is meant to demonstrate how to implement the Hessian-Schatten norm with Pyxu (the Schatten norm could be
  ...
  in 2D images).
* The Condat-Vu algorithm converges very slowly in these examples; a stricter stopping criterion leads to significantly
  better (and slower) reconstructions.
...
.. code-block:: python3
   ...
   import pycsou.opt.solver as pxs
   import pycsou.opt.stop as pxst

   class NuclearNorm(pxa.ProxFunc):
...
def _array_module_to_sparse(array_module: pxd.NDArrayInfo) -> pxd.SparseArrayInfo:
    r"""
    Returns the suitable sparse array module that is compatible with the input array module.
    """
    if array_module == pxd.NDArrayInfo.CUPY:
        sparse_module = pxd.SparseArrayInfo.CUPY_SPARSE
    elif array_module == pxd.NDArrayInfo.DASK:
        sparse_module = pxd.SparseArrayInfo.PYDATA_SPARSE
    else:
        sparse_module = pxd.SparseArrayInfo.SCIPY_SPARSE
    return sparse_module


def _to_sparse_backend(
    spmat: pxt.SparseArray,
    sparse_module: pxd.SparseArrayInfo = pxd.SparseArrayInfo.SCIPY_SPARSE,
):
    r"""
    Returns sparse array module compatible with the current array module.
    """
    if sparse_module == pxd.SparseArrayInfo.CUPY_SPARSE:
        spmat = pxd.SparseArrayInfo.CUPY_SPARSE.module().csr_matrix(spmat)
    elif sparse_module == pxd.SparseArrayInfo.PYDATA_SPARSE:
        spmat = pxd.SparseArrayInfo.PYDATA_SPARSE.module().GCXS(spmat)
    return spmat


def _convert_to_list(*args, ndim: pxt.Integer = 1):
    r"""
    Converts a collection of objects that are either lists of length ndim or single objects into a tuple of lists.
    Single objects are converted to lists of ndim instances of this object. ndim is either provided or inferred from the
    length of the provided lists.
    """
    out = ()
...
def _BSplineDerivative(
    ...
    degrees: typ.Union[pxt.Integer, list[pxt.Integer]],
    deriv_orders: typ.Union[pxt.Integer, list[pxt.Integer]],
    ndim: pxt.Integer = 1,
    array_module: pxd.NDArrayInfo = pxd.NDArrayInfo.NUMPY,
) -> pxa.LinOp:
    r"""
    ...
    ndim: int, optional
        Number of dimensions, `i.e.`, of variables of the spline (can be inferred from previous parameters if a list is
        provided).
    ...
    for i in range(ndim):
        if deriv_orders[i] > 0:
            bspline = sci.BSpline(
                t=knots[i], c=np.eye(knots[i].size - degrees[i] - 1, dtype=pxrt.getPrecision().value), k=degrees[i]
            )
            op = sp.csr_array(
            ...
            if array_module == pxd.NDArrayInfo.DASK:
                op = da.from_array(op)
            op = pxa.LinOp.from_array(op.astype(pxrt.getPrecision().value))
        else:
            op = pxo.IdentityOp(dim=knots[i].size - degrees[i] - 1)
        if i == 0:
            op_ndim = op
        else:
            op_ndim = pxo.kron(op_ndim, op)
    return op_ndim


# @pxrt.enforce_precision('samples', o=False)
def BSplineSampling(
    eval_grid: typ.Union[np.ndarray, list[np.ndarray]],
    knots: typ.Union[np.ndarray, list[np.ndarray]],
    degrees: typ.Union[pxt.Integer, list[pxt.Integer]],
    deriv_orders: typ.Union[pxt.Integer, list[pxt.Integer]] = 0,
    ndim: pxt.Integer = 1,
    array_module: pxd.NDArrayInfo = pxd.NDArrayInfo.NUMPY,
) -> pxa.LinOp:
    r"""
    ...
    The univariate B-spline basis functions :math:`B_{j_d, k_d; \mathbf{t}[d]}` with :math:`0 \leq d \leq D-1` are
    defined as in the documentation of Scipy's :py:class:`~scipy.interpolate.BSpline` class.
    ...
    for i in range(ndim):
        ...
        op = pxa.LinOp.from_array(spmat.astype(pxrt.getPrecision().value))
        if i == 0:
            op_ndim = op
        else:
            op_ndim = pxo.kron(op_ndim, op)
...
def _precond_nonuniform_knots(
    ...
    ndim: pxt.Integer = 1,
    array_module: pxd.NDArrayInfo = pxd.NDArrayInfo.NUMPY,
) -> tuple[pxa.LinOp, pxa.LinOp, pxt.NDArray]:
    r"""
    ...
    :math:`\mathbf{c}`; replacing :math:`\mathbf{c}` with :math:`\mathbf{P}\tilde{\mathbf{c}}` in the problem
    formulation and optimizing over :math:`\tilde{\mathbf{c}}` will improve the numerical conditioning of the problem.
    The spline coefficients can then be recovered via :math:`\mathbf{c}^\ast = \mathbf{P} \tilde{\mathbf{c}}^\ast`,
    where :math:`\tilde{\mathbf{c}}^\ast` is the obtained solution of the preconditioned problem.
    ...
    for i in range(ndim):
        ...
        weights = xp.asarray(degrees[i] / (knots[i][degrees[i] : -1] - knots[i][:n]), dtype=pxrt.getPrecision().value)
        precond = pxo.DiagonalOp(weights)
        precond_inv = pxo.DiagonalOp(1 / weights)
        if i == 0:
            precond_ndim, precond_inv_ndim, weights_ndim = precond, precond_inv, weights
        else:
            precond_ndim = pxo.kron(precond_ndim, precond)
            precond_inv_ndim = pxo.kron(precond_inv_ndim, precond_inv)
            weights_ndim = xp.kron(weights_ndim, weights)
    return precond_ndim, precond_inv_ndim, weights_ndim


@pxrt.enforce_precision(["knots", "precond_weights"], o=False)
def _periodic_constraints(
    knots: np.ndarray,
    degree: pxt.Integer,
    precond_weights: pxt.NDArray = 1,
) -> tuple[pxa.LinOp, pxa.LinOp]:
    r"""
    Computes the linear operator of constraints on univariate B-spline coefficients for this spline to be periodic.  A
    spline is periodic if and only if this operator applied to its B-spline coefficients outputs zero.
    ...
    P: :py:class:`~pycsou.abc.operator.LinOp`
        ...
        :py:class:`~pycsou.operator.linop.bspline.BSplinePeriodicIndFunc`).
    """
    xp = pxu.get_array_module(knots)
    t = np.asarray(knots, dtype=pxrt.getPrecision().value)
    interval = (t[degree], t[-degree - 1])  # Bounds of the period
    w = np.asarray(precond_weights, dtype=pxrt.getPrecision().value)
    rows = []
    for i in range(degree):
        if i == 0:  # s(0) = s(T)
            ...
            sDk = _BSplineDerivative(
                knots=knots, degrees=degree, deriv_orders=i, array_module=pxd.NDArrayInfo.from_obj(knots)
            )
    ...
    P = np.eye(A.shape[-1], dtype=pxrt.getPrecision().value) - Adag @ A
    A, P = xp.asarray(A, dtype=pxrt.getPrecision().value), xp.asarray(P, dtype=pxrt.getPrecision().value)
    return pxa.LinOp.from_array(A), pxa.LinOp.from_array(P)


class BSplinePeriodicIndFunc(pxo.ShiftLossMixin, pxa.ProxFunc):
    r"""
    ...
    :math:`T`-periodic at the boundary points :math:`t[d]_{k_d}` and :math:`t[d]_{-k_d}` with :math:`T = t[d]_{-k_d} -
    t[d]_{k_d}`, `i.e.`,
    ...
    By default, the operator :math:`\mathbf{A}` encodes the periodicity of :math:`S` over all its variables. However, it
    may only do so for a subset of its variables.
    ...
    def __init__(
        ...
        precond_weights: typ.Union[pxt.NDArray, list[pxt.NDArray]] = 1,
        ndim: pxt.Integer = 1,
    ):
        r"""
        Parameters
        ...
        precond_weights: ndarray or list[ndarray], optional
            ...
            :py:func:`~pycsou.operator.linop.bspline._precond_nonuniform_knots`); list if different for each dimension.
            Default is 1 (no preconditioning).
        ndim: int, optional
        ...
        for i in range(ndim):
            ...
            if is_periodic[i]:
                ...
                _A_list.append(pxo.NullOp(shape=(0, shape_in[-1])))
                _P_list.append(pxo.IdentityOp(dim=shape_in[-1]))
...
def uniform_knots(
    nb_of_knots: pxt.Integer,
    t_min: pxt.Real,
    t_max: pxt.Real,
    degree: pxt.Integer,
) -> np.ndarray:
    r"""
    Returns uniform array of knots based on limit points of a base interval. Includes boundary points outside the base
    interval so that a spline with these knots has the correct base interval (see Scipy's
...
# @pxrt.enforce_precision('eval_grid', o=False)
def BSplineGradientGrid(
    ...
    for i in range(ndim):
        ...
        deriv_orders[i] = 0
    return pxo.vstack(grad_op_list)
...
def BSplineLaplacianGrid(
    ...
    op: :py:class:`~pycsou.abc.operator.LinOp`,
        shape :math:`\left(\prod_{d=0}^{D-1} m_d, \prod_{d=0}^{D-1} n_d \right)`.
        Laplacian operator.
...
def BSplineHessianGrid(
    ...
    with shape :math:`\left(\frac{D (D+1)}2 \times \prod_{d=0}^{D-1} m_d, \right)`. The vector :math:`\mathbf{H}(j_0,
    \ldots, j_{D-1})` corresponds to the stacked upper triangular elements of the Hessian matrix of the spline
    ...
    where the notation :math:`\partial_{x_d} \partial_{x_{d'}} S` is a shorthand for :math:`\partial_{x_d}
    \partial_{x_{d'}} S \big((\mathbf{x}[0])_{j_0}, \ldots,(\mathbf{x}[D-1])_{j_{D-1}} \big)` (see
    :py:func:`~pycsou.operator.linop.bspline.BSplineSampling` for notations).
    ...
    for i in range(ndim):
        ...
        for j in range(ndim - i):
            ...
            deriv_orders[j + i] -= 1
        deriv_orders[i] = 0
    return pxo.vstack(hess_op_list)


def _is_valid_knots(knots: np.ndarray, degree: pxt.Integer, bc_type: str = None):
    r"""
...
class _PiecewiseCstInnos1D(pxa.LinOp):
    r"""
...
# class _PiecewiseCstInnos(pxa.LinOp):
#     r"""
#     Linear operator that takes B-spline coefficients of a piecewise-constant spline and outputs its innovations.
#     """
#
#     def __init__(self, shape_in: tuple[pxt.Integer, ...], bc_types: typ.Union[str, list[str]] = None):
#         self.shape_in = shape_in
...
def BSplineInnos1D(
    ...
    bc_type: str, optional
        ...
        * ``"not-a-knot"`` (default): The spline has no knots at the limit points, which amounts to constant
          extrapolation. This is equivalent to ``None``.

        * ``"periodic"``: The spline has a knot at the starting point (identified with the end point with periodic
          boundary conditions).
    ...
    Notes
    -----
    The `innovation` of a spline :math:`S` of degree :math:`k` is the :math:`(k+1)`-th derivative of :math:`S`, which in
    common cases is a Dirac comb of the form
    ...
    the case where :math:`S` has uniform knots (see [2]). The vector :math:`\mathbf{a}` then represents the amplitudes
    of :math:`S` at the knots:
    ...
    The operator ``op`` is typically used in combination with an :py:class:`~pycsou.operator.func.norm.L1Norm` due to
    the relation
    ...
    **Remark**
    This function requires all the `internal` knots of :math:`S` to have multiplicity 1, `i.e.`, :math:`t_{k+1}, \ldots,
    t_{-(k+1)}` must be pairwise distinct. This condition is equivalent to the innovation :math:`w` being a Dirac comb
    as in Eq. :math:numref:`eq:inno` (see [BSplineDeBoor]_).

    Examples
    --------
    Consider the problem of interpolating :math:`M` 1D data points :math:`(x_m, y_m)_{1 \leq m \leq M}` with a sparse
    spline or degree :math:`k`, `i.e.`, a spline with few knots. To achieve this, we formulate the following
    ...
    .. plot::
       ...
       import pycsou.operator.linop.bspline as bsp
       import pycsou.operator as pycop
       import pycsou.operator.func.norm as pxo
