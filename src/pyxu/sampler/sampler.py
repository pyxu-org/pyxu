...
also be computed in an online fashion (see :py:mod:`~pyxu.sampler.statistics` module) and provide useful diagnostic
tools for uncertainty quantification.

In the following example, we showcase the unajusted Langevin algorithm (:py:class:`~pyxu.sampler.sampler.ULA`) applied
...
.. code-block:: python3
    ...
    from pyxu.sampler.sampler import ULA
    import pyxu.sampler.statistics as pxst
    ...
    f = 1 / 2 * pxf.SquaredL2Norm(dim=N).argshift(-y.ravel()) / sigma ** 2  # Data fidelity loss
    ...
    online_mean = pxst.OnlineMoment(order=1)
    online_var = pxst.OnlineVariance()
...
class ULA(_Sampler):
    ...
    * Consecutive samples are typically correlated, which can deteriorate the Monte-Carlo estimation of quantities of
    ...
    .. plot::
        ...
        import pyxu.operator.func as pxf
        from pyxu.sampler.sampler import ULA
        from pyxu.sampler.statistics import OnlineMoment, OnlineVariance

        f = pxf.SquaredL2Norm(dim=1) / 2  # To sample 1D normal distribution (mean 0, variance 1)
...
class MYULA(ULA):
    ...
    *Lipschitz continuous* gradient, and :math:`\mathcal{G}: \mathbb{R}^N \to \mathbb{R}` is *proper*, *lower semi-
    ...
    Notes
    -----
    MYULA is an extension of :py:class:`~pyxu.sampler.sampler.ULA` to sample from distributions whose logarithm is
    ...
    of :py:class:`~pyxu.sampler.sampler.ULA` documentation). MYULA is guaranteed to converges when :math:`\gamma \leq
    ...
    def __init__(
        ...
        gamma: float | None
            ...
            :py:class:`~pyxu.sampler.sampler.ULA` documentation).
        lambda: float | None
        ...
        self._f_diff = pxf.NullFunc(dim=dim) if (f is None) else f
        self._g = pxf.NullFunc(dim=dim) if (g is None) else g
