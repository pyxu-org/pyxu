...
def _resolve(p: str | plib.Path) -> plib.Path:
    return plib.Path(p).expanduser().resolve()


def xdg_config_root() -> plib.Path:
    return _resolve(os.getenv("XDG_CONFIG_HOME", "~/.config"))


def xdg_data_root() -> plib.Path:
    return _resolve(os.getenv("XDG_DATA_HOME", "~/.local/share"))


def xdg_cache_root() -> plib.Path:
    return _resolve(os.getenv("XDG_CACHE_HOME", "~/.cache"))
