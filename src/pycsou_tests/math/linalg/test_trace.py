...
class TestTrace:
    ...
    def test_value_explicit(self, op, _op_trace):
        tr = pylinalg.trace(op)
        assert ct.allclose(tr, _op_trace, as_dtype=pycrt.getPrecision().value)
    ...
    def test_value_hutchpp(self, op, _op_trace):
        ...
        N_trial = 100
        tr = np.array([pylinalg.hutchpp(op) for _ in range(N_trial)])
