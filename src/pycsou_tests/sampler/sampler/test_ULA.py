...
class TestULA(ct.SamplerT):
    ...
    @pytest.fixture
    def func(self, dim):
        return 1 / 2 * pycop.SquaredL2Norm(dim=dim)  # Sampling of standard normal distribution

    @pytest.fixture
    def sampler(self, func):
        return pycs.ULA(f=func)
    ...
    def samples_copy(self, func, sampler, seed, x0_np, xp, width):
        ...
        sampler_copy = pycs.ULA(f=func)  # Create copy of sampler
...
class TestMYULA(TestULA):
    ...
    def prox_func(self, dim):
        return pycop.L1Norm(dim=dim)

    @pytest.fixture
    def sampler(self, func, prox_func):
        return pycs.MYULA(f=func, g=prox_func)
    ...
    def samples_copy(self, func, prox_func, sampler, seed, x0_np, xp, width):
        ...
        rng_2 = xp.random.default_rng(seed=seed)
        sampler._rng = rng_1  # Reset rng object of sampler
        sampler_copy = pycs.MYULA(f=func, g=prox_func)  # Create copy of sampler
