...
class SamplerT(ct.DisableTestMixin):
    ...
    # Fixtures ----------------------------------------------------------------
    @pytest.fixture
    def spec(self) -> tuple[typ.Generator, pycd.NDArrayInfo, pycrt.Width]:
    ...
    @pytest.fixture
    def samples_list(self, num_samples, samples) -> list[pyct.NDArray]:
        with pycrt.EnforcePrecision(False):
