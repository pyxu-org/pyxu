...
def spec_op_map(klass: pyct.OpC, N: int = 2) -> list[list[pyct.OpT]]:
    ...
    def condition(ops: list[pyct.OpT], klass: pyct.OpC) -> bool:
        ...
        if (pyco.Property.LINEAR in properties) and (codim == dim):
            properties = pyco.SquareOp.properties()
        else:
            properties = properties & pyco.LinOp.properties()

        _klass = pyco.Operator._infer_operator_type(properties)
        return _klass == klass
...
def spec_op_func(klass: pyct.OpC, N: int = 2) -> list[list[pyct.OpT]]:
    # create all possible (op1, ..., opN) tuples which, when used to create a hstack-ed operator,
    ...
    def condition(ops: list[pyct.OpT], klass: pyct.OpC) -> bool:
        ...
        for op in ops[1:]:
            op_sum = op_sum + op
        _klass = pyco.Operator._infer_operator_type(op_sum.properties())
        return _klass == klass
...
class TestHStackQuadraticFunc(HStackFuncMixin, conftest.QuadraticFuncT):
    @pytest.fixture(params=spec_op_func(pyco.QuadraticFunc))
    def op_all(self, request):
        return request.param
