...
class KhatriRaoMixin:
    ...
    # Fixtures ----------------------------------------------------------------
    @pytest.fixture
    def op_AB(self) -> tuple[pyct.OpT, pyct.OpT]:
        # Override in inherited class with A/B operands.
        raise NotImplementedError

    @pytest.fixture
    def op_A(self, op_AB) -> pyct.OpT:
        return op_AB[0]

    @pytest.fixture
    def op_B(self, op_AB) -> pyct.OpT:
        return op_AB[1]
    ...
    def spec(self, op_A, op_B, request) -> tuple[pyct.OpT, pycd.NDArrayInfo, pycrt.Width]:
        ndi, width = request.param
        op = pycl.khatri_rao(op_A, op_B)
        return op, ndi, width
