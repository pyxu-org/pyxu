...
class TestFFT(conftest.LinOpT):
    ...
    @pytest.fixture
    def spec(self, _spec, transform_real) -> tuple[pyct.OpT, pycd.NDArrayInfo, pycrt.Width]:
    ...
    @pytest.fixture
    def data_shape(self, arg_shape, transform_real) -> pyct.OpShape:
    ...
    def data_apply(self, arg_shape, axes, transform_real) -> conftest.DataLike:
        ...
        if transform_real:
            ...
            arr = arr_r + 1j * arr_c
            arr_gt = pycu.view_as_real(arr.reshape(-1))

        out = np.fft.fftn(arr, axes=axes, norm="backward")
        out_gt = pycu.view_as_real(out.reshape(-1))
