...
class TestStructureTensor(conftest.DiffMapT):
    ...
    def init_params(self, diff_method, sampling, request):
        ...
        if diff_method == "fd":
            return pyctd.diff_params_fd(params_fd[0], params_fd[1], sampling)
        elif diff_method == "gd":
            return pyctd.diff_params_gd(params_gd[0], params_gd[1], sampling)
    ...
    @pytest.fixture
    def spec(self, arg_shape, filter_kwargs, ndi, width) -> tuple[pyct.OpT, pycd.NDArrayInfo, pycrt.Width]:
        with pycrt.Precision(width):
    ...
    def data_apply(self, arg_shape, diff_method, gt_diffs, filter_kwargs) -> conftest.DataLike:
        ...
        derivatives = pyctd.apply_gradient(
            arr,
            arg_shape=arg_shape,
            gt_diffs=gt_diffs,
        ...
        gt_smooth = pyctd.diff_params_gd(
