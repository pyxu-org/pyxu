...
class TestPartialDerivative(DiffOpMixin):
    ...
    def diff_op(self, diff_method):
        if diff_method == "fd":
            return pycld.PartialDerivative.finite_difference
        elif diff_method == "gd":
            return pycld.PartialDerivative.gaussian_derivative
...
class TestJacobian(DiffOpMixin):
    ...
    @pytest.fixture
    def data_shape(self, arg_shape, n_channels, directions) -> pyct.NDArrayShape:
        size = np.prod(arg_shape).item()
        n_derivatives = len(directions) if directions is not None else len(arg_shape)
