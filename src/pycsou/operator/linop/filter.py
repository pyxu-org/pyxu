...
def Gaussian(
    ...
    for i, (sigma_, truncate_, order_) in enumerate(zip(sigma, truncate, order)):
        if sigma_:
            radius = int(truncate_ * float(sigma_) + 0.5)
...
def _EdgeFilter(
    ...
    for edge_dim in axes:
        ...
        kernel[edge_dim] = xp.array([-1, 0, 1], dtype=dtype)
...
class StructureTensor(pyco.DiffMap):
    ...
    [TODO] In case of using finite differences, use `central` method (see :py:class:`~pycsou.operator.linop.diff.PartialDerivative`).
