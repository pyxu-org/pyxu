...
def from_jax(
    ...
    * If provided, arithmetic methods must abide exactly to the interface below:
      ...
      .. code-block:: python3
         ...
         def prox(arr: jax.Array, tau: pyct.Real) -> jax.Array
         def pinv(arr: jax.Array, damp: pyct.Real) -> jax.Array
    ...
    * All arithmetic methods provided in `kwargs` are decorated using
      ...
      This can be achieved locally using the :py:class:`~pycsou.runtime.Precision` context manager.
      (See example.)
      Alternatively, precision/copy-related warnings can be silenced via the `enable_warnings`
      option of :py:func:`~pycsou.operator.interop.jax.from_jax`.

    * Inferred arithmetic methods are not JIT-ed by default since the operation is error-prone
      depending on how :py:meth:`~pycsou.abc.operator.Map.apply` is defined.
      If :py:meth:`~pycsou.abc.operator.Map.apply` supplied to
      :py:func:`~pycsou.operator.interop.from_jax` is JIT-friendly, then consider enabling `jit`.
    ...
    def __init__(  # See from_jax() for a detailed description.
        ...
        **kwargs,
        ...
        super().__init__(
            ...
            vectorize=vectorize,
            vmethod=None,  # pycsou.util.vectorize() not used for jax-funcs
