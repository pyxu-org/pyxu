...
def salt_and_pepper_noise(arr, percent, svp_percent):
    # assume its between 0 and 1 the values
    assert 0 < percent < 1.0
    size = arr.size
    salt = int(size * percent * svp_percent)
    pepper = int(size * percent * (1-svp_percent))
    # easily choose same coord then it just gets overwritten and not really keeping consistent with the percent.
    salt_coord = np.random.choice(size, size=salt, replace=False)
    pepper_coord = np.random.choice(size, size=pepper, replace=False)
    arr[salt_coord] = 1
    arr[pepper_coord] = 0
    return arr