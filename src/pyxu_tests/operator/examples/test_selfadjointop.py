...
class SelfAdjointConvolution(pxa.SelfAdjointOp):
    # Convolution (along last axis) where filter coefficients imply operator is self-adjoint.
...
class TestSelfAdjointConvolution(conftest.SelfAdjointOpT):
    ...
    def spec(self, dim_shape, request) -> tuple[pxt.OpT, pxd.NDArrayInfo, pxrt.Width]:
        ...
        op = SelfAdjointConvolution(dim_shape)
        return op, ndi, width
