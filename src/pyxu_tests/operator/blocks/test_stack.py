# How stack() tests work:
#
# * StackMixin auto-defines all (input,output) pairs.
#   [Caveat: we assume the base operators (op_all) are correctly implemented.]
#   (True if choosing test operators from examples/.)
#
# * To test a stack-ed operator, inherit from StackMixin and the suitable conftest.MapT subclass
...
def spec_op(klass: pxt.OpC, N: int = 2) -> list[list[pxt.OpT]]:
    # create all possible (op1, ..., opN) tuples which, when used to create a stacked operator,
    ...
    def condition(ops: list[pxt.OpT], klass: pxt.OpC) -> bool:
        # Return true if stack(ops) forms a klass object. [Not a sub-type]
...
# Test classes (Maps) ---------------------------------------------------------
class TestStackMap(StackMixin, conftest.MapT):
    @pytest.fixture(params=spec_op(pxa.Map))
    def op_all(self, request):
        return request.param


class TestStackDiffMap(StackMixin, conftest.DiffMapT):
    @pytest.fixture(params=spec_op(pxa.DiffMap))
    def op_all(self, request):
        return request.param


class TestStackLinOp(StackMixin, conftest.LinOpT):
