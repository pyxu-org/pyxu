...
class KhatriRaoProduct(LinearOperator):
    ...
    def __call__(self, x: np.ndarray) -> np.ndarray:
        if self.linop1.is_explicit and self.linop2.is_explicit:
            return np.asarray(self.linop2.multiply(x[None, :]).dot(self.linop1.mat.transpose()))
        else:
    ...
    def adjoint(self, y: np.ndarray) -> np.ndarray:
        Y = y.reshape((self.linop2.shape[0], self.linop1.shape[0]))
        if self.linop1.is_explicit and self.linop2.is_explicit:
            return np.asarray(self.linop1.mat.transpose().conj().dot(Y.transpose()).transpose().multiply(self.linop2.mat.conj()).sum(axis=0))
        else:
            return np.diag(
